...
$resultsXPos = 366 * $screenScale
$resultsYPos = 548 * $screenScale
$resultsFoundColor = "ff1f1f1f"

Read-Host -Prompt "Press Enter to start searching test"

for ($i = -1; $i -lt $iterations; $i++) {
    Set-CursorPosition $repeatFindXPos $repeatFindYPos
    ...
    while (! $resultsFound) {
        ...
        if ($color.Name -ne $resultsFoundColor) {
            $endingTime = Get-Date
            $resultsFound = $true
        }
    }
    ...
    if ($i -eq -1) {
        Write-Output "Skipping first result with $searchTime ms"
    }
    else {
        # Write to file
